    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="space-y-3">
          <Label htmlFor="mass1" className="text-cyan-400">Mass (Type 1) = {params.mass1}</Label>
          <Slider id="mass1" min={1} max={20} step={1} value={[params.mass1]} onValueChange={([v]) => handleSliderChange('mass1', v)} />
        </div>
        <div className="space-y-3">
          <Label htmlFor="count1" className="text-cyan-400">Count (Type 1) = {params.count1}</Label>
          <Slider id="count1" min={1} max={1000} step={1} value={[params.count1]} onValueChange={([v]) => handleSliderChange('count1', v)} />
        </div>
        <div className="space-y-3">
          <Label htmlFor="mass2" className="text-fuchsia-400">Mass (Type 2) = {params.mass2}</Label>
          <Slider id="mass2" min={1} max={20} step={1} value={[params.mass2]} onValueChange={([v]) => handleSliderChange('mass2', v)} />
        </div>
         <div className="space-y-3">
          <Label htmlFor="count2" className="text-fuchsia-400">Count (Type 2) = {params.count2}</Label>
